...
resource "azurerm_spring_cloud_service" "resname" {
  ...
  container_registry {
    name = var.spring_cloud_service_container_registry_name
    password = var.spring_cloud_service_container_registry_password
    server = var.spring_cloud_service_container_registry_server
    username = var.spring_cloud_service_container_registry_username
  }

  default_build_service {
    #container_registry_name = var.spring_cloud_service_default_build_service_container_registry_name
  }

  marketplace {
    plan = var.spring_cloud_service_marketplace_plan
    product = var.spring_cloud_service_marketplace_product
    publisher = var.spring_cloud_service_marketplace_publisher
  }

  network {
    app_subnet_id = var.spring_cloud_service_network_app_subnet_id
    cidr_ranges = var.spring_cloud_service_network_cidr_ranges
    #outbound_type = var.spring_cloud_service_network_outbound_type
