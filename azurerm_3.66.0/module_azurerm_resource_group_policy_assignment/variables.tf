/*This terraform file has been generated programmatically using terraform-generator.*/
/*All the commented lines, if any, are optional. Remove comment characters, if required, before using.*/
/*Refer https://github.com/sushil46in/terraform_modules.git for more details.*/

/*variable "resource_group_policy_assignment_description" {
  description = "(Optional)" #The value for description
  type = string
}*/

/*variable "resource_group_policy_assignment_display_name" {
  description = "(Optional)" #The value for display_name
  type = string
}*/

/*variable "resource_group_policy_assignment_enforce" {
  description = "(Optional)" #The value for enforce
  type = bool
}*/

/*variable "resource_group_policy_assignment_location" {
  description = "(Optional)" #The value for location
  type = string
}*/

variable "resource_group_policy_assignment_name" {
  description = "(Required)" #The value for name
  type = string
}

/*variable "resource_group_policy_assignment_not_scopes" {
  description = "(Optional)" #The value for not_scopes
  type = list
}*/

/*variable "resource_group_policy_assignment_parameters" {
  description = "(Optional)" #The value for parameters
  type = string
}*/

variable "resource_group_policy_assignment_policy_definition_id" {
  description = "(Required)" #The value for policy_definition_id
  type = string
}

variable "resource_group_policy_assignment_resource_group_id" {
  description = "(Required)" #The value for resource_group_id
  type = string
}

/*variable "resource_group_policy_assignment_identity_identity_ids" {
  description = "(Optional)" #The value for identity_identity_ids
  type = set
}*/

variable "resource_group_policy_assignment_identity_type" {
  description = "(Required)" #The value for identity_type
  type = string
}

variable "resource_group_policy_assignment_non_compliance_message_content" {
  description = "(Required)" #The value for non_compliance_message_content
  type = string
}

/*variable "resource_group_policy_assignment_non_compliance_message_policy_definition_reference_id" {
  description = "(Optional)" #The value for non_compliance_message_policy_definition_reference_id
  type = string
}*/

variable "resource_group_policy_assignment_overrides_value" {
  description = "(Required)" #The value for overrides_value
  type = string
}

/*variable "resource_group_policy_assignment_selectors_in" {
  description = "(Optional)" #The value for selectors_in
  type = list
}*/

/*variable "resource_group_policy_assignment_selectors_not_in" {
  description = "(Optional)" #The value for selectors_not_in
  type = list
}*/

/*variable "resource_group_policy_assignment_resource_selectors_name" {
  description = "(Optional)" #The value for resource_selectors_name
  type = string
}*/

/*variable "resource_group_policy_assignment_selectors_in" {
  description = "(Optional)" #The value for selectors_in
  type = list
}*/

variable "resource_group_policy_assignment_selectors_kind" {
  description = "(Required)" #The value for selectors_kind
  type = string
}

/*variable "resource_group_policy_assignment_selectors_not_in" {
  description = "(Optional)" #The value for selectors_not_in
  type = list
}*/

/*variable "resource_group_policy_assignment_timeouts_create" {
  description = "(Optional)" #The value for timeouts_create
  type = string
}*/

/*variable "resource_group_policy_assignment_timeouts_delete" {
  description = "(Optional)" #The value for timeouts_delete
  type = string
}*/

/*variable "resource_group_policy_assignment_timeouts_read" {
  description = "(Optional)" #The value for timeouts_read
  type = string
}*/

/*variable "resource_group_policy_assignment_timeouts_update" {
  description = "(Optional)" #The value for timeouts_update
  type = string
}*/

