/*This terraform file has been generated programmatically using terraform-generator.*/
/*All the commented lines, if any, are optional. Remove comment characters, if required, before using.*/
/*Refer https://github.com/sushil46in/terraform-generator.git for more details.*/

variable "network_security_rule_access" {
  description = "(Required) The value for access"
  type = string
}

/*variable "network_security_rule_description" {
  description = "(Optional) The value for description"
  type = string
}*/

/*variable "network_security_rule_destination_address_prefix" {
  description = "(Optional) The value for destination_address_prefix"
  type = string
}*/

/*variable "network_security_rule_destination_address_prefixes" {
  description = "(Optional) The value for destination_address_prefixes"
  type = set
}*/

/*variable "network_security_rule_destination_application_security_group_ids" {
  description = "(Optional) The value for destination_application_security_group_ids"
  type = set
}*/

/*variable "network_security_rule_destination_port_range" {
  description = "(Optional) The value for destination_port_range"
  type = string
}*/

/*variable "network_security_rule_destination_port_ranges" {
  description = "(Optional) The value for destination_port_ranges"
  type = set
}*/

variable "network_security_rule_direction" {
  description = "(Required) The value for direction"
  type = string
}

variable "network_security_rule_name" {
  description = "(Required) The value for name"
  type = string
}

variable "network_security_rule_network_security_group_name" {
  description = "(Required) The value for network_security_group_name"
  type = string
}

variable "network_security_rule_priority" {
  description = "(Required) The value for priority"
  type = number
}

variable "network_security_rule_protocol" {
  description = "(Required) The value for protocol"
  type = string
}

variable "network_security_rule_resource_group_name" {
  description = "(Required) The value for resource_group_name"
  type = string
}

/*variable "network_security_rule_source_address_prefix" {
  description = "(Optional) The value for source_address_prefix"
  type = string
}*/

/*variable "network_security_rule_source_address_prefixes" {
  description = "(Optional) The value for source_address_prefixes"
  type = set
}*/

/*variable "network_security_rule_source_application_security_group_ids" {
  description = "(Optional) The value for source_application_security_group_ids"
  type = set
}*/

/*variable "network_security_rule_source_port_range" {
  description = "(Optional) The value for source_port_range"
  type = string
}*/

/*variable "network_security_rule_source_port_ranges" {
  description = "(Optional) The value for source_port_ranges"
  type = set
}*/

/*variable "network_security_rule_timeouts_create" {
  description = "(Optional) The value for timeouts_create"
  type = string
}*/

/*variable "network_security_rule_timeouts_delete" {
  description = "(Optional) The value for timeouts_delete"
  type = string
}*/

/*variable "network_security_rule_timeouts_read" {
  description = "(Optional) The value for timeouts_read"
  type = string
}*/

/*variable "network_security_rule_timeouts_update" {
  description = "(Optional) The value for timeouts_update"
  type = string
}*/

