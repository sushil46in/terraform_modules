/*This terraform file has been generated programmatically using terraform-generator.*/
/*All the commented lines, if any, are optional. Remove comment characters, if required, before using.*/
/*Refer https://github.com/sushil46in/terraform-generator.git for more details.*/

variable "automation_connection_automation_account_name" {
  description = "(Required) The value for automation_account_name"
  type = string
}

/*variable "automation_connection_description" {
  description = "(Optional) The value for description"
  type = string
}*/

variable "automation_connection_name" {
  description = "(Required) The value for name"
  type = string
}

variable "automation_connection_resource_group_name" {
  description = "(Required) The value for resource_group_name"
  type = string
}

variable "automation_connection_type" {
  description = "(Required) The value for type"
  type = string
}

variable "automation_connection_values" {
  description = "(Required) The value for values"
  type = map
}

/*variable "automation_connection_timeouts_create" {
  description = "(Optional) The value for timeouts_create"
  type = string
}*/

/*variable "automation_connection_timeouts_delete" {
  description = "(Optional) The value for timeouts_delete"
  type = string
}*/

/*variable "automation_connection_timeouts_read" {
  description = "(Optional) The value for timeouts_read"
  type = string
}*/

/*variable "automation_connection_timeouts_update" {
  description = "(Optional) The value for timeouts_update"
  type = string
}*/

