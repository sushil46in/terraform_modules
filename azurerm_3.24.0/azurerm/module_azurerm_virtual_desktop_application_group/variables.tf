/*This terraform file has been generated programmatically using terraform-generator.*/
/*All the commented lines, if any, are optional. Remove comment characters, if required, before using.*/
/*Refer https://github.com/sushil46in/terraform-generator.git for more details.*/

/*variable "virtual_desktop_application_group_default_desktop_display_name" {
  description = "(Optional) The value for default_desktop_display_name"
  type = string
}*/

/*variable "virtual_desktop_application_group_description" {
  description = "(Optional) The value for description"
  type = string
}*/

/*variable "virtual_desktop_application_group_friendly_name" {
  description = "(Optional) The value for friendly_name"
  type = string
}*/

variable "virtual_desktop_application_group_host_pool_id" {
  description = "(Required) The value for host_pool_id"
  type = string
}

variable "virtual_desktop_application_group_location" {
  description = "(Required) The value for location"
  type = string
}

variable "virtual_desktop_application_group_name" {
  description = "(Required) The value for name"
  type = string
}

variable "virtual_desktop_application_group_resource_group_name" {
  description = "(Required) The value for resource_group_name"
  type = string
}

/*variable "virtual_desktop_application_group_tags" {
  description = "(Optional) The value for tags"
  type = map
}*/

variable "virtual_desktop_application_group_type" {
  description = "(Required) The value for type"
  type = string
}

/*variable "virtual_desktop_application_group_timeouts_create" {
  description = "(Optional) The value for timeouts_create"
  type = string
}*/

/*variable "virtual_desktop_application_group_timeouts_delete" {
  description = "(Optional) The value for timeouts_delete"
  type = string
}*/

/*variable "virtual_desktop_application_group_timeouts_read" {
  description = "(Optional) The value for timeouts_read"
  type = string
}*/

/*variable "virtual_desktop_application_group_timeouts_update" {
  description = "(Optional) The value for timeouts_update"
  type = string
}*/

