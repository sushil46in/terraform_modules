/*This terraform file has been generated programmatically using terraform-generator.*/
/*All the commented lines, if any, are optional. Remove comment characters, if required, before using.*/
/*Refer https://github.com/sushil46in/terraform-generator.git for more details.*/

variable "dns_steering_policy_attachment_domain_name" {
  description = "(Required)" #The value for domain_name
  type = string
}

variable "dns_steering_policy_attachment_steering_policy_id" {
  description = "(Required)" #The value for steering_policy_id
  type = string
}

variable "dns_steering_policy_attachment_zone_id" {
  description = "(Required)" #The value for zone_id
  type = string
}

/*variable "dns_steering_policy_attachment_timeouts_create" {
  description = "(Optional)" #The value for timeouts_create
  type = string
}*/

/*variable "dns_steering_policy_attachment_timeouts_delete" {
  description = "(Optional)" #The value for timeouts_delete
  type = string
}*/

/*variable "dns_steering_policy_attachment_timeouts_update" {
  description = "(Optional)" #The value for timeouts_update
  type = string
}*/

