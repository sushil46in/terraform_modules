...
resource "google_compute_node_group" "resname" {
  ...
  share_settings {
    share_type = var.compute_node_group_share_settings_share_type
    project_map {
      id = var.compute_node_group_project_map_id
      project_id = var.compute_node_group_project_map_project_id
    }
  }
