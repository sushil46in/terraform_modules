...
variable "storage_bucket_autoclass_enabled" {
  description = "(Required)" #While set to true, autoclass automatically transitions objects in your bucket to appropriate storage classes based on each object's access pattern.
  type = bool
}
...
variable "storage_bucket_custom_placement_config_data_locations" {
  description = "(Required)" #The list of individual regions that comprise a dual-region bucket. See the docs for a list of acceptable regions. Note: If any of the data_locations changes, it will recreate the bucket.
  type = set
}
...
variable "storage_bucket_action_type" {
  description = "(Required)" #The type of the action of this Lifecycle Rule. Supported values include: Delete, SetStorageClass and AbortIncompleteMultipartUpload.
  type = string
}
