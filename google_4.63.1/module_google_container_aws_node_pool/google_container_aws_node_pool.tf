...
resource "google_container_aws_node_pool" "resname" {
  ...
  config {
    ...
    autoscaling_metrics_collection {
      granularity = var.container_aws_node_pool_autoscaling_metrics_collection_granularity
      #metrics = var.container_aws_node_pool_autoscaling_metrics_collection_metrics
    }
    config_encryption {
