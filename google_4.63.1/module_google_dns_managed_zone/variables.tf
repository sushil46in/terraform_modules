...
/*variable "dns_managed_zone_force_destroy" {
  description = "(Optional)" #Set this true to delete all records in the zone.
  type = bool
}*/
...
/*variable "dns_managed_zone_visibility" {
  description = "(Optional)" #The zone's visibility: public zones are exposed to the Internet, while private zones are visible only to Virtual Private Cloud resources. Default value: "public" Possible values: ["private", "public"]
  type = string
}*/

variable "dns_managed_zone_cloud_logging_config_enable_logging" {
...
variable "dns_managed_zone_gke_clusters_gke_cluster_name" {
...
variable "dns_managed_zone_networks_network_url" {
  description = "(Required)" #The id or fully qualified URL of the VPC network to bind to. This should be formatted like 'projects/{project}/global/networks/{network}' or 'https://www.googleapis.com/compute/v1/projects/{project}/global/networks/{network}'
  type = string
}
