...
variable "bigtable_instance_name" {
  description = "(Required)" #The name (also called Instance Id in the Cloud Console) of the Cloud Bigtable instance. Must be 6-33 characters and must only contain hyphens, lowercase letters and numbers.
  type = string
}

variable "bigtable_instance_cluster_cluster_id" {
  description = "(Required)" #The ID of the Cloud Bigtable cluster. Must be 6-30 characters and must only contain hyphens, lowercase letters and numbers.
  type = string
}
