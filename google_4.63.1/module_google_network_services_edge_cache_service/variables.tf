...
variable "network_services_edge_cache_service_response_header_to_remove_header_name" {
  description = "(Required)" #Headers to remove from the response prior to sending it back to the client.  Response headers are only sent to the client, and do not have an effect on the cache serving the response.
  type = string
}
...
/*variable "network_services_edge_cache_service_cdn_policy_signed_request_maximum_expiration_ttl" {
  description = "(Optional)" #Limit how far into the future the expiration time of a signed request may be.  When set, a signed request is rejected if its expiration time is later than now + signedRequestMaximumExpirationTtl, where now is the time at which the signed request is first handled by the CDN.  - The TTL must be > 0. - Fractions of a second are not allowed.  By default, signedRequestMaximumExpirationTtl is not set and the expiration time of a signed request may be arbitrarily far into future.
  type = string
}*/

variable "network_services_edge_cache_service_add_signatures_actions" {
  description = "(Required)" #The actions to take to add signatures to responses. Possible values: ["GENERATE_COOKIE", "GENERATE_TOKEN_HLS_COOKIELESS", "PROPAGATE_TOKEN_HLS_COOKIELESS"]
  type = list
}

/*variable "network_services_edge_cache_service_add_signatures_copied_parameters" {
...
/*variable "network_services_edge_cache_service_add_signatures_keyset" {
...
/*variable "network_services_edge_cache_service_add_signatures_token_query_parameter" {
...
/*variable "network_services_edge_cache_service_add_signatures_token_ttl" {
...
/*variable "network_services_edge_cache_service_signed_token_options_allowed_signature_algorithms" {
  description = "(Optional)" #The allowed signature algorithms to use.  Defaults to using only ED25519.  You may specify up to 3 signature algorithms to use. Possible values: ["ED25519", "HMAC_SHA_256", "HMAC_SHA1"]
  type = list
}*/

/*variable "network_services_edge_cache_service_signed_token_options_token_query_parameter" {
