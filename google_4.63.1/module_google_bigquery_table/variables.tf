...
/*variable "bigquery_table_external_data_configuration_reference_file_schema_uri" {
...
variable "bigquery_table_avro_options_use_avro_logical_types" {
  description = "(Required)" #If sourceFormat is set to "AVRO", indicates whether to interpret logical types as the corresponding BigQuery data type (for example, TIMESTAMP), instead of using the raw type (for example, INTEGER).
  type = bool
}
