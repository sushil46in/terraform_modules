...
/*variable "pubsub_subscription_enable_exactly_once_delivery" {
  description = "(Optional) If 'true', Pub/Sub provides the following guarantees for the delivery
...
/*variable "pubsub_subscription_bigquery_config_drop_unknown_fields" {
  description = "(Optional) When true and useTopicSchema is true, any fields that are a part of the topic schema that are not part of the BigQuery table schema are dropped when writing to BigQuery.
...
/*variable "pubsub_subscription_bigquery_config_write_metadata" {
  description = "(Optional) When true, write the subscription name, messageId, publishTime, attributes, and orderingKey to additional columns in the table.
The subscription name, messageId, and publishTime fields are put in their own columns while all other message properties (other than data) are written to a JSON object in the attributes column."
  type = bool
}*/

/*variable "pubsub_subscription_dead_letter_policy_dead_letter_topic" {
  description = "(Optional) The name of the topic to which dead letter messages should be published.
...
/*variable "pubsub_subscription_dead_letter_policy_max_delivery_attempts" {
  description = "(Optional) The maximum number of delivery attempts for any message. The value must be
between 5 and 100.
...
variable "pubsub_subscription_expiration_policy_ttl" {
  description = "(Required) Specifies the "time-to-live" duration for an associated resource. The
...
/*variable "pubsub_subscription_push_config_attributes" {
  description = "(Optional) Endpoint configuration attributes.
...
variable "pubsub_subscription_push_config_push_endpoint" {
  description = "(Required) A URL locating the endpoint to which messages should be pushed.
...
/*variable "pubsub_subscription_oidc_token_audience" {
  description = "(Optional) Audience to be used when generating OIDC token. The audience claim
...
variable "pubsub_subscription_oidc_token_service_account_email" {
  description = "(Required) Service account email to be used for generating the OIDC token.
