...
variable "bigquery_dataset_dataset_target_types" {
  description = "(Required) Which resources in the dataset this entry applies to. Currently, only views are supported,
...
variable "bigquery_dataset_view_table_id" {
  description = "(Required) The ID of the table. The ID must contain only letters (a-z,
...
variable "bigquery_dataset_default_encryption_configuration_kms_key_name" {
  description = "(Required) Describes the Cloud KMS encryption key that will be used to protect destination
