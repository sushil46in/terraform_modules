...
/*variable "bigquery_routine_return_table_type" {
  description = "(Optional) Optional. Can be set only if routineType = "TABLE_VALUED_FUNCTION".
...
/*variable "bigquery_routine_arguments_data_type" {
  description = "(Optional) A JSON schema for the data type. Required unless argumentKind = ANY_TYPE.
