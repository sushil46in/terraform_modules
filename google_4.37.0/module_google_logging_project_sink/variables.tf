...
/*variable "logging_project_sink_unique_writer_identity" {
  description = "(Optional) 'Whether or not to create a unique identity associated with this sink. If false (the default), then the writer_identity used is serviceAccount:cloud-logs@system.gserviceaccount.com. If true, then a unique service account is created and used for this sink. If you wish to publish logs across projects, you must set unique_writer_identity to true.'"
  type = bool
}*/

variable "logging_project_sink_bigquery_options_use_partitioned_tables" {
  description = "(Required) 'Whether to use BigQuery's partition tables. By default, Logging creates dated tables based on the log entries' timestamps, e.g. syslog_20170523. With partitioned tables the date suffix is no longer present and special query syntax has to be used instead. In both cases, tables are sharded based on UTC timezone.'"
  type = bool
}

/*variable "logging_project_sink_exclusions_description" {
  description = "(Optional) 'A description of this exclusion.'"
  type = string
}*/

/*variable "logging_project_sink_exclusions_disabled" {
  description = "(Optional) 'If set to True, then this exclusion is disabled and it does not exclude any log entries'"
  type = bool
}*/

variable "logging_project_sink_exclusions_filter" {
...
variable "logging_project_sink_exclusions_name" {
