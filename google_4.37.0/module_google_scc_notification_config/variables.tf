...
variable "scc_notification_config_organization" {
  description = "(Required) The organization whose Cloud Security Command Center the Notification
Config lives in."
  type = string
}

variable "scc_notification_config_pubsub_topic" {
  description = "(Required) The Pub/Sub topic to send notifications to. Its format is
"projects/[project_id]/topics/[topic]"."
  type = string
}

variable "scc_notification_config_streaming_config_filter" {
  description = "(Required) Expression that defines the filter to apply across create/update
