...
/*variable "dataproc_metastore_service_location" {
  description = "(Optional) The location where the metastore service should reside.
...
variable "dataproc_metastore_service_encryption_config_kms_key" {
  description = "(Required) The fully qualified customer provided Cloud KMS key name to use for customer data encryption.
...
variable "dataproc_metastore_service_keytab_cloud_secret" {
  description = "(Required) The relative resource name of a Secret Manager secret version, in the following form:
