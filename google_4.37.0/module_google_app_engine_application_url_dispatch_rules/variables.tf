...
/*variable "app_engine_application_url_dispatch_rules_dispatch_rules_domain" {
  description = "(Optional) Domain name to match against. The wildcard "*" is supported if specified before a period: "*.".
Defaults to matching all domains: "*"."
  type = string
}*/

variable "app_engine_application_url_dispatch_rules_dispatch_rules_path" {
  description = "(Required) Pathname within the host. Must start with a "/". A single "*" can be included at the end of the path.
