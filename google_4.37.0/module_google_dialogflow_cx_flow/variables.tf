...
/*variable "dialogflow_cx_flow_language_code" {
  description = "(Optional) The language of the following fields in flow:
...
/*variable "dialogflow_cx_flow_parent" {
  description = "(Optional) The agent to create a flow for.
Format: projects/<Project ID>/locations/<Location ID>/agents/<Agent ID>."
  type = string
}*/

/*variable "dialogflow_cx_flow_transition_route_groups" {
  description = "(Optional) A flow's transition route group serve two purposes:
...
/*variable "dialogflow_cx_flow_nlu_settings_classification_threshold" {
  description = "(Optional) To filter out false positive results and still get variety in matched natural language inputs for your agent, you can tune the machine learning classification threshold.
If the returned score value is less than the threshold value, then a no-match event will be triggered. The score values range from 0.0 (completely uncertain) to 1.0 (completely certain). If set to 0.0, the default of 0.3 is used."
  type = number
}*/

/*variable "dialogflow_cx_flow_nlu_settings_model_training_mode" {
  description = "(Optional) Indicates NLU model training mode.
...
/*variable "dialogflow_cx_flow_nlu_settings_model_type" {
  description = "(Optional) Indicates the type of NLU model.
