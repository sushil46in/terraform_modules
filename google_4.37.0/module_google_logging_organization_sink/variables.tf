...
variable "logging_organization_sink_name" {
  description = "(Required)" #The name of the logging sink.
  type = string
}

variable "logging_organization_sink_org_id" {
  description = "(Required)" #The numeric ID of the organization to be exported to the sink.
  type = string
}

variable "logging_organization_sink_bigquery_options_use_partitioned_tables" {
  description = "(Required)" #Whether to use BigQuery's partition tables. By default, Logging creates dated tables based on the log entries' timestamps, e.g. syslog_20170523. With partitioned tables the date suffix is no longer present and special query syntax has to be used instead. In both cases, tables are sharded based on UTC timezone.
  type = bool
}

/*variable "logging_organization_sink_exclusions_description" {
  description = "(Optional)" #A description of this exclusion.
  type = string
}*/

/*variable "logging_organization_sink_exclusions_disabled" {
  description = "(Optional)" #If set to True, then this exclusion is disabled and it does not exclude any log entries
  type = bool
}*/

variable "logging_organization_sink_exclusions_filter" {
...
variable "logging_organization_sink_exclusions_name" {
