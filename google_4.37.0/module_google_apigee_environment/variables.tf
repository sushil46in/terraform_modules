...
/*variable "apigee_environment_node_config_max_node_count" {
  description = "(Optional) The maximum total number of gateway nodes that the is reserved for all instances that
...
/*variable "apigee_environment_node_config_min_node_count" {
  description = "(Optional) The minimum total number of gateway nodes that the is reserved for all instances that
has the specified environment. If not specified, the default is determined by the
