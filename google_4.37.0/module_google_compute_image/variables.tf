...
/*variable "compute_image_raw_disk_container_type" {
  description = "(Optional) The format used to encode and transmit the block device, which
...
/*variable "compute_image_raw_disk_sha1" {
  description = "(Optional) An optional SHA1 checksum of the disk image before unpackaging.
This is provided by the client when the disk image is created."
  type = string
}*/

variable "compute_image_raw_disk_source" {
  description = "(Required) The full Google Cloud Storage URL where disk storage is stored
