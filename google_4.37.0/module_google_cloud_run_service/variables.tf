...
variable "cloud_run_service_containers_image" {
  description = "(Required) Docker image name. This is most often a reference to a container located
...
/*variable "cloud_run_service_containers_working_dir" {
  description = "(Optional) Container's working directory.
...
/*variable "cloud_run_service_env_value" {
  description = "(Optional) Variable references $(VAR_NAME) are expanded
...
variable "cloud_run_service_secret_key_ref_key" {
  description = "(Required) A Cloud Secret Manager secret version. Must be 'latest' for the latest
version or an integer for a specific version."
  type = string
}

variable "cloud_run_service_secret_key_ref_name" {
  description = "(Required) The name of the secret in Cloud Secret Manager. By default, the secret is assumed to be in the same project. 
...
/*variable "cloud_run_service_resources_requests" {
  description = "(Optional) Requests describes the minimum amount of compute resources required.
...
variable "cloud_run_service_volume_mounts_mount_path" {
  description = "(Required) Path within the container at which the volume should be mounted.  Must
not contain ':'."
  type = string
}
...
/*variable "cloud_run_service_secret_default_mode" {
  description = "(Optional) Mode bits to use on created files by default. Must be a value between 0000
...
variable "cloud_run_service_secret_secret_name" {
  description = "(Required) The name of the secret in Cloud Secret Manager. By default, the secret
...
variable "cloud_run_service_items_key" {
  description = "(Required) The Cloud Secret Manager secret version.
Can be 'latest' for the latest value or an integer for a specific version."
  type = string
}

/*variable "cloud_run_service_items_mode" {
  description = "(Optional) Mode bits to use on this file, must be a value between 0000 and 0777. If
...
variable "cloud_run_service_items_path" {
  description = "(Required) The relative path of the file to map the key to.
...
/*variable "cloud_run_service_traffic_latest_revision" {
  description = "(Optional) LatestRevision may be optionally provided to indicate that the latest ready
