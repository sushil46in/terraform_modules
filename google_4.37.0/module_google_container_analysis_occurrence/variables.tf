...
variable "container_analysis_occurrence_attestation_serialized_payload" {
  description = "(Required) The serialized payload that is verified by one or
more signatures. A base64-encoded string."
  type = string
}

variable "container_analysis_occurrence_signatures_public_key_id" {
  description = "(Required) The identifier for the public key that verifies this
...
/*variable "container_analysis_occurrence_signatures_signature" {
  description = "(Optional) The content of the signature, an opaque bytestring.
