...
variable "pubsub_lite_topic_retention_config_per_partition_bytes" {
  description = "(Required) The provisioned storage, in bytes, per partition. If the number of bytes stored
...
/*variable "pubsub_lite_topic_retention_config_period" {
  description = "(Optional) How long a published message is retained. If unset, messages will be retained as
