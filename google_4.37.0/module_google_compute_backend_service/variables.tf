...
/*variable "compute_backend_service_negative_caching_policy_ttl" {
  description = "(Optional) The TTL (in seconds) for which to cache responses with the corresponding status code. The maximum allowed value is 1800s
...
variable "compute_backend_service_security_settings_client_tls_policy" {
  description = "(Required) ClientTlsPolicy is a resource that specifies how a client should authenticate
...
variable "compute_backend_service_security_settings_subject_alt_names" {
  description = "(Required) A list of alternate names to verify the subject identity in the certificate.
