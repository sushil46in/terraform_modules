...
/*variable "data_loss_prevention_job_trigger_output_config_output_schema" {
  description = "(Optional) Schema used for writing the findings for Inspect jobs. This field is only used for
...
/*variable "data_loss_prevention_job_trigger_table_table_id" {
  description = "(Optional) Name of the table. If is not set a new one will be generated for you with the following format:
...
/*variable "data_loss_prevention_job_trigger_cloud_storage_options_bytes_limit_per_file" {
  description = "(Optional) Max number of bytes to scan from a file. If a scanned file's size is bigger than this value
then the rest of the bytes are omitted."
  type = number
}*/

/*variable "data_loss_prevention_job_trigger_cloud_storage_options_bytes_limit_per_file_percent" {
  description = "(Optional) Max percentage of bytes to scan from a file. The rest are omitted. The number of bytes scanned is rounded down.
Must be between 0 and 100, inclusively. Both 0 and 100 means no limit."
  type = number
}*/

/*variable "data_loss_prevention_job_trigger_cloud_storage_options_file_types" {
  description = "(Optional) List of file type groups to include in the scan. If empty, all files are scanned and available data
...
/*variable "data_loss_prevention_job_trigger_cloud_storage_options_files_limit_percent" {
  description = "(Optional) Limits the number of files to scan to this percentage of the input FileSet. Number of files scanned is rounded down.
Must be between 0 and 100, inclusively. Both 0 and 100 means no limit."
  type = number
}*/

/*variable "data_loss_prevention_job_trigger_cloud_storage_options_sample_method" {
  description = "(Optional) How to sample bytes if not all bytes are scanned. Meaningful only when used in conjunction with bytesLimitPerFile.
If not specified, scanning would start from the top. Possible values: ["TOP", "RANDOM_START"]"
  type = string
}*/

/*variable "data_loss_prevention_job_trigger_file_set_url" {
  description = "(Optional) The Cloud Storage url of the file(s) to scan, in the format 'gs://<bucket>/<path>'. Trailing wildcard
...
/*variable "data_loss_prevention_job_trigger_regex_file_set_exclude_regex" {
  description = "(Optional) A list of regular expressions matching file paths to exclude. All files in the bucket that match at
least one of these regular expressions will be excluded from the scan."
  type = list
}*/

/*variable "data_loss_prevention_job_trigger_regex_file_set_include_regex" {
  description = "(Optional) A list of regular expressions matching file paths to include. All files in the bucket
...
/*variable "data_loss_prevention_job_trigger_timespan_config_enable_auto_population_of_timespan_config" {
  description = "(Optional) When the job is started by a JobTrigger we will automatically figure out a valid startTime to avoid
...
variable "data_loss_prevention_job_trigger_timestamp_field_name" {
  description = "(Required) Specification of the field containing the timestamp of scanned items. Used for data sources like Datastore and BigQuery.
...
/*variable "data_loss_prevention_job_trigger_schedule_recurrence_period_duration" {
  description = "(Optional) With this option a job is started a regular periodic basis. For example: every day (86400 seconds).
