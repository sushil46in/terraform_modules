...
/*variable "compute_router_encrypted_interconnect_router" {
  description = "(Optional) Field to indicate if a router is dedicated to use with encrypted
...
/*variable "compute_router_bgp_advertised_groups" {
  description = "(Optional) User-specified list of prefix groups to advertise in custom mode.
...
variable "compute_router_bgp_asn" {
  description = "(Required) Local BGP Autonomous System Number (ASN). Must be an RFC6996
...
/*variable "compute_router_bgp_keepalive_interval" {
  description = "(Optional) The interval in seconds between BGP keepalive messages that are sent to the peer.
...
variable "compute_router_advertised_ip_ranges_range" {
  description = "(Required) The IP range to advertise. The value must be a
