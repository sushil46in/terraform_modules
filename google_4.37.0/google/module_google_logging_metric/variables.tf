...
/*variable "logging_metric_metric_descriptor_display_name" {
  description = "(Optional) A concise name for the metric, which can be displayed in user interfaces. Use sentence case 
...
variable "logging_metric_metric_descriptor_metric_kind" {
  description = "(Required) Whether the metric records instantaneous values, changes to a value, etc.
...
/*variable "logging_metric_metric_descriptor_unit" {
  description = "(Optional) The unit in which the metric value is reported. It is only applicable if the valueType is
...
variable "logging_metric_metric_descriptor_value_type" {
  description = "(Required) Whether the measurement is an integer, a floating-point number, etc.
Some combinations of metricKind and valueType might not be supported.
