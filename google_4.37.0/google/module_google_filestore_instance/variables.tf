...
variable "filestore_instance_file_shares_capacity_gb" {
  description = "(Required) File share capacity in GiB. This must be at least 1024 GiB
...
/*variable "filestore_instance_nfs_export_options_access_mode" {
  description = "(Optional) Either READ_ONLY, for allowing only read requests on the exported directory,
...
/*variable "filestore_instance_nfs_export_options_anon_gid" {
  description = "(Optional) An integer representing the anonymous group id with a default value of 65534.
...
/*variable "filestore_instance_nfs_export_options_anon_uid" {
  description = "(Optional) An integer representing the anonymous user id with a default value of 65534.
...
/*variable "filestore_instance_nfs_export_options_ip_ranges" {
  description = "(Optional) List of either IPv4 addresses, or ranges in CIDR notation which may mount the file share.
...
/*variable "filestore_instance_nfs_export_options_squash_mode" {
  description = "(Optional) Either NO_ROOT_SQUASH, for allowing root access on the exported directory, or ROOT_SQUASH,
for not allowing root access. The default is NO_ROOT_SQUASH. Default value: "NO_ROOT_SQUASH" Possible values: ["NO_ROOT_SQUASH", "ROOT_SQUASH"]"
  type = string
}*/

/*variable "filestore_instance_networks_connect_mode" {
  description = "(Optional) The network connect mode of the Filestore instance.
...
variable "filestore_instance_networks_modes" {
  description = "(Required) IP versions for which the instance has
IP addresses assigned. Possible values: ["ADDRESS_MODE_UNSPECIFIED", "MODE_IPV4", "MODE_IPV6"]"
  type = list
}

variable "filestore_instance_networks_network" {
  description = "(Required) The name of the GCE VPC network to which the
