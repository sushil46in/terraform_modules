...
/*variable "bigquery_job_copy_create_disposition" {
  description = "(Optional) Specifies whether the job is allowed to create new tables. The following values are supported:
...
/*variable "bigquery_job_copy_write_disposition" {
  description = "(Optional) Specifies the action that occurs if the destination table already exists. The following values are supported:
...
variable "bigquery_job_destination_encryption_configuration_kms_key_name" {
  description = "(Required) Describes the Cloud KMS encryption key that will be used to protect destination BigQuery table.
The BigQuery Service Account associated with your project requires access to this encryption key."
  type = string
}

variable "bigquery_job_destination_table_table_id" {
  description = "(Required) The table. Can be specified '{{table_id}}' if 'project_id' and 'dataset_id' are also set,
...
/*variable "bigquery_job_extract_compression" {
  description = "(Optional) The compression type to use for exported files. Possible values include GZIP, DEFLATE, SNAPPY, and NONE.
...
/*variable "bigquery_job_load_allow_jagged_rows" {
  description = "(Optional) Accept rows that are missing trailing optional columns. The missing values are treated as nulls.
...
/*variable "bigquery_job_load_allow_quoted_newlines" {
  description = "(Optional) Indicates if BigQuery should allow quoted data sections that contain newline characters in a CSV file.
...
/*variable "bigquery_job_load_ignore_unknown_values" {
  description = "(Optional) Indicates if BigQuery should allow extra values that are not represented in the table schema.
...
/*variable "bigquery_job_load_max_bad_records" {
  description = "(Optional) The maximum number of bad records that BigQuery can ignore when running the job. If the number of bad records exceeds this value,
an invalid error is returned in the job result. The default value is 0, which requires that all records are valid."
  type = number
}*/

/*variable "bigquery_job_load_null_marker" {
  description = "(Optional) Specifies a string that represents a null value in a CSV file. For example, if you specify "\N", BigQuery interprets "\N" as a null value
...
/*variable "bigquery_job_load_projection_fields" {
  description = "(Optional) If sourceFormat is set to "DATASTORE_BACKUP", indicates which entity properties to load into BigQuery from a Cloud Datastore backup.
...
/*variable "bigquery_job_load_schema_update_options" {
  description = "(Optional) Allows the schema of the destination table to be updated as a side effect of the load job if a schema is autodetected or
...
/*variable "bigquery_job_load_skip_leading_rows" {
  description = "(Optional) The number of rows at the top of a CSV file that BigQuery will skip when loading the data.
...
/*variable "bigquery_job_load_source_format" {
  description = "(Optional) The format of the data files. For CSV files, specify "CSV". For datastore backups, specify "DATASTORE_BACKUP".
...
variable "bigquery_job_load_source_uris" {
  description = "(Required) The fully-qualified URIs that point to your data in Google Cloud.
...
/*variable "bigquery_job_time_partitioning_field" {
  description = "(Optional) If not set, the table is partitioned by pseudo column '_PARTITIONTIME'; if set, the table is partitioned by this field.
...
variable "bigquery_job_time_partitioning_type" {
  description = "(Required) The only type supported is DAY, which will generate one partition per day. Providing an empty string used to cause an error,
but in OnePlatform the field will be treated as unset."
  type = string
}

/*variable "bigquery_job_query_allow_large_results" {
  description = "(Optional) If true and query uses legacy SQL dialect, allows the query to produce arbitrarily large result tables at a slight cost in performance.
...
/*variable "bigquery_job_query_flatten_results" {
  description = "(Optional) If true and query uses legacy SQL dialect, flattens all nested and repeated fields in the query results.
allowLargeResults must be true if this is set to false. For standard SQL queries, this flag is ignored and results are never flattened."
  type = bool
}*/

/*variable "bigquery_job_query_maximum_billing_tier" {
  description = "(Optional) Limits the billing tier for this job. Queries that have resource usage beyond this tier will fail (without incurring a charge).
If unspecified, this will be set to your project default."
  type = number
}*/

/*variable "bigquery_job_query_maximum_bytes_billed" {
  description = "(Optional) Limits the bytes billed for this job. Queries that will have bytes billed beyond this limit will fail (without incurring a charge).
If unspecified, this will be set to your project default."
  type = string
}*/
...
/*variable "bigquery_job_query_schema_update_options" {
  description = "(Optional) Allows the schema of the destination table to be updated as a side effect of the query job.
...
/*variable "bigquery_job_query_use_legacy_sql" {
  description = "(Optional) Specifies whether to use BigQuery's legacy SQL dialect for this query. The default value is true.
If set to false, the query will use BigQuery's standard SQL."
  type = bool
}*/

/*variable "bigquery_job_query_use_query_cache" {
  description = "(Optional) Whether to look for the result in the query cache. The query cache is a best-effort cache that will be flushed whenever
...
variable "bigquery_job_default_dataset_dataset_id" {
  description = "(Required) The dataset. Can be specified '{{dataset_id}}' if 'project_id' is also set,
...
/*variable "bigquery_job_script_options_key_result_statement" {
  description = "(Optional) Determines which statement in the script represents the "key result",
...
/*variable "bigquery_job_user_defined_function_resources_inline_code" {
  description = "(Optional) An inline resource that contains code for a user-defined function (UDF).
