...
/*variable "app_engine_flexible_app_version_service_account" {
  description = "(Optional) The identity that the deployed version will run as. Admin API will use the App Engine Appspot service account as
...
/*variable "app_engine_flexible_app_version_automatic_scaling_cool_down_period" {
  description = "(Optional) The time period that the Autoscaler should wait before it starts collecting information from a new instance.
...
/*variable "app_engine_flexible_app_version_cloud_build_options_cloud_build_timeout" {
  description = "(Optional) The Cloud Build timeout used as part of any dependent builds performed by version creation. Defaults to 10 minutes.
...
variable "app_engine_flexible_app_version_container_image" {
  description = "(Required) URI to the hosted container image in Google Container Registry. The URI must be fully qualified and include a tag or digest.
...
/*variable "app_engine_flexible_app_version_endpoints_api_service_config_id" {
  description = "(Optional) Endpoints service configuration ID as specified by the Service Management API. For example "2016-09-19r1".
...
variable "app_engine_flexible_app_version_endpoints_api_service_name" {
  description = "(Required) Endpoints service name which is the name of the "service" resource in the Service Management API.
...
/*variable "app_engine_flexible_app_version_static_files_application_readable" {
  description = "(Optional) Whether files should also be uploaded as code data. By default, files declared in static file handlers are
...
/*variable "app_engine_flexible_app_version_static_files_path" {
  description = "(Optional) Path to the static files matched by the URL pattern, from the application root directory.
...
/*variable "app_engine_flexible_app_version_network_subnetwork" {
  description = "(Optional) Google Cloud Platform sub-network where the virtual machines are created. Specify the short name, not the resource path.
...
/*variable "app_engine_flexible_app_version_readiness_check_app_start_timeout" {
  description = "(Optional) A maximum time limit on application initialization, measured from moment the application successfully
