...
/*variable "pubsub_topic_message_retention_duration" {
  description = "(Optional) Indicates the minimum duration to retain a message after it is published
...
variable "pubsub_topic_message_storage_policy_allowed_persistence_regions" {
  description = "(Required) A list of IDs of GCP regions where messages that are published to
