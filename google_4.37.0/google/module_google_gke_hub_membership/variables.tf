...
variable "gke_hub_membership_authority_issuer" {
  description = "(Required) A JSON Web Token (JWT) issuer URI. 'issuer' must start with 'https://' and // be a valid 
...
variable "gke_hub_membership_gke_cluster_resource_link" {
  description = "(Required) Self-link of the GCP resource for the GKE cluster.
