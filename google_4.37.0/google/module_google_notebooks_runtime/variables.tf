...
/*variable "notebooks_runtime_virtual_machine_config_nic_type" {
  description = "(Optional) The type of vNIC to be used on this interface. This may be gVNIC or VirtioNet. Possible values: ["UNSPECIFIED_NIC_TYPE", "VIRTIO_NET", "GVNIC"]"
  type = string
}*/

/*variable "notebooks_runtime_virtual_machine_config_reserved_ip_range" {
  description = "(Optional) Reserved IP Range name is used for VPC Peering. The subnetwork allocation will use the range *name* if it's assigned."
  type = string
}*/
...
/*variable "notebooks_runtime_data_disk_type" {
  description = "(Optional) Specifies the type of the disk, either SCRATCH or PERSISTENT. If not specified, the default is PERSISTENT."
  type = string
}*/
...
/*variable "notebooks_runtime_encryption_config_kms_key" {
  description = "(Optional) The Cloud KMS resource identifier of the customer-managed encryption key used to protect a resource, such as a disks. It has the following format: 'projects/{PROJECT_ID}/locations/{REGION}/keyRings/ {KEY_RING_NAME}/cryptoKeys/{KEY_NAME}'"
  type = string
}*/
