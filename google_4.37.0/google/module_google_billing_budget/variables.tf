...
/*variable "billing_budget_all_updates_rule_disable_default_iam_recipients" {
  description = "(Optional) Boolean. When set to true, disables default notifications sent
...
/*variable "billing_budget_all_updates_rule_monitoring_notification_channels" {
  description = "(Optional) The full resource name of a monitoring notification
channel in the form
...
/*variable "billing_budget_all_updates_rule_pubsub_topic" {
  description = "(Optional) The name of the Cloud Pub/Sub topic where budget related
...
/*variable "billing_budget_all_updates_rule_schema_version" {
  description = "(Optional) The schema version of the notification. Only "1.0" is
...
/*variable "billing_budget_amount_last_period_amount" {
  description = "(Optional) Configures a budget amount that is automatically set to 100% of
last period's spend.
...
/*variable "billing_budget_specified_amount_units" {
  description = "(Optional) The whole units of the amount. For example if currencyCode
is "USD", then 1 unit is one US dollar."
  type = string
}*/

/*variable "billing_budget_budget_filter_calendar_period" {
  description = "(Optional) A CalendarPeriod represents the abstract concept of a recurring time period that has a
...
/*variable "billing_budget_budget_filter_credit_types_treatment" {
  description = "(Optional) Specifies how credits should be treated when determining spend
...
/*variable "billing_budget_budget_filter_projects" {
  description = "(Optional) A set of projects of the form projects/{project_number},
...
/*variable "billing_budget_threshold_rules_spend_basis" {
  description = "(Optional) The type of basis used to determine if spend has passed
...
variable "billing_budget_threshold_rules_threshold_percent" {
  description = "(Required) Send an alert when this threshold is exceeded. This is a
